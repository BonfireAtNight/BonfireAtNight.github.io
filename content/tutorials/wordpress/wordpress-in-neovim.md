...
Neovim verfügt über eine Reihe von Features, um damit produktiv für WordPress entwickeln zu können. Oder besser gesagt: Neovim lässt sich so konfigurieren, dass von äußerst hilfreichen Werkzeugen Gebrauch gemacht werden kann, die sich ihrem generellen Design nach in verschiedenen Texteditoren einbetten lassen.

In diesem Artikel werde ich über einige dieser Werkzeuge sprechen. Ich werde erklären, inwiefern sie uns bei der Arbeit mit WordPress helfen und wie man sie für eine möglichst komfortable Arbeit in Neovim integriert.

Ihre Installation ist leider mit einigen Fallstricken versehen. Sie ergeben sich nicht zuletzt daraus, dass verschiedene Paketsysteme die Werkzeuge an verschiedenen Orten abspeichern. Da ich selbst mit einer Arch-basierten Linux-Distribution arbeite, werde ich das Setup dafür vornehmen.


## Intelephense: LSP-Features

### Ziel: Funktionen wie in einer integrierten Entwicklungsumgebung (IDE)
Neovim stellt von Haus aus einen Client für das sogenannte [Language Server Protocol](https://microsoft.github.io/language-server-protocol/){: target="_blank"} bereit. Dabei handelt es sich um einen nicht zuletzt von Microsoft beförderten Standard. Eine Vielzahl von sprachabhängigen Features können durch externe Language Server implementiert werden.

Genug der allgemeinen Worte, was heißt das für WordPress? Das Content-Management-System selbst wie auch seine Themes und Plugins werden primär in PHP geschrieben. Das heißt wir brauchen einen Language Server für diese Sprache. Soweit ich sehe haben wir vor allem zwei Optionen: [phpactor](https://github.com/phpactor/phpactor){: target="_blank"} und [Intelephense](https://intelephense.com/){: target="_blank"}.

Beide werben damit, dass sie eine Reihe von Features bieten, wie man sie von einer integrierten Entwicklungsumgebung erwarten würde. Sie versorgen die Entwicklerin mit Informationen aus der offiziellen PHP-Dokumentation, sie geben eine Diagnose des Codes und weisen auf Probleme hin, sie bieten eine smarte Rename-Funktion und sie erlauben es, sich schnell durch die Codebasis zu bewegen (go to definition, go to declaration, go to type definition etc.). Code kann nach Standards wie PSR12 automatisch formatiert werden. Und über sogenannte Code-Actions lassen sich DocStrings und notwendige Importe automatisch einfügen.

Würde es uns nur um PHP gehen, würden die Unterschiede vielleicht keine allzu große Rolle spielen. Für WordPress fällt die Wahl eindeutig auf Intelephense. Der Grund dafür liegt darin, dass dieser Language Server über sogenannte PHP-Stubs lernen kann, welche Bezeichner (für Variablen, Konstanten, Klassen, Methoden etc.) im Rahmen von WordPress definiert wurden. Ohne diese Informationen würde unser der Language Server hundertfach darauf hinweisen, dass ihm die verwendeten Symbole unbekannt sind. Außerdem können wir so Informationen aus der Dokumentation abrufen, wie wir es für die im PHP-Core eingebauten Komponenten bereits konnten.
...
Zwar ist npm nicht auf Node.js beschränkt und dürfte auf dem Computer vieler Webentwickler installiert sein.[^npm] Doch der Language Server kann über den PHP-Paketmanager, [Composer](https://getcomposer.org/){: target="_blank"}, installiert werden. Wir installieren Composer mit Pacman und installieren damit im Anschluss Intelephense global auf dem Computer.
```bash
pacman -S composer
composer global require bmewburn/intelephense
```

Einige Pakete erwarten, über Composer installierte Anwendungen direkt aufrufen zu können. Zumindest zur Sicherheit empfiehlt es sich deshalb, das Unterverzeichnis mit den ausführbaren Binärdateien der `PATH`-Umgebungsvariable hinzuzufügen.[^composer-pfad] Das geschieht in der Shell-Konfiguration.
```bash
export PATH="$PATH:$HOME/.config/composer/vendor/bin"
```

Über Composer können wir nun auch die für WordPress benötigten PHP-Stubs installieren. Wir könnten sie individuell als Projekt-Dependencies anfordern. Der Einfachheit halber installiere ich sie global:
```bash
...
Davon werden die meisten nur einige wenige benötigen: WordPress-Core, WordPress-Globals und vielleicht noch Woocommerce. Es ist aber vielleicht interessant zu wissen, dass es Stubs auch für andere populäre Plugins gibt.

Schließlich brauchen wir Composer noch aus einem anderen Grund. Intelephense wird sich nur dann mit unserem Code-Buffer verbinden können, wenn es das Wurzelverzeichnis des Projekts identifizieren kann, zu dem die geöffnete Datei gehört. Dazu wird ein Elternverzeichnis gesucht, in der sich eine `composer.json` befindet.

Für kleinere Projekte ohne nennenswerte Dependencies kann die Konfiguration sehr spartanisch sein. Für ernsthaftere Projekte kann über `composer init` ein Setup-Assistent gestartet werden.

Natürlich sind bei der Plugin- und Theme-Entwicklung andere Sprachen beteiligt. HTML und CSS spielen notwendig eine Rolle. Für Frontend-Entwicklung wird sicher auch JavaScript geschrieben werden. Hier geht es mir erst einmal nur um WordPress selbst. In Zukunft werde ich sicher weitere Beiträge zum Thema Neovim und Webentwicklung schreiben.

### Intelephense in Neovim nutzen
Als nächsten Schritt müssen wir Neovim für die WordPress-Entwicklung konfigurieren. Das geschieht primär dadurch, dass Intelephense als Language Server für die PHP-Entwicklung festgelegt wird. Darübe hinasu müssen wir festlegen, welche PHP-Stubs verwendet werden sollen und gegebenenfalls, wo es diese Stubs findet.
...
```
Natürlich müssen die beiden Pfade für den eigenen Benutzer abgewandelt werden. Bei den Stubs muss man entscheiden, welche man verwenden möchte. Die `capabilities`-Zeile dient dazu, Completion-Voschläge zu erhalten.

Wenn man nun eine PHP-Datei öffnet, sollte sich der Buffer automatisch mit Intelephense verbinden. Um das zu kontrollieren, kann `:LspInfo` aufgerufen werden. Code-Diganosen Completion-Vorschläge dürften damit bereits ohne Weiteres angezeigt werden.

Andere Informationen und Aktionen müssen ausdrücklich angefordert bzw. ausgelöst werden. Da man das natürlich nicht über die Kommandoschnittstelle machen möchte, müssen Keybindings definiert werden. Falls man das nicht sowieso schon für die Verwendung eines anderen Language Servers gemacht hat, muss man sie selbst festlegen:
```lua
-- Global Mappings
...
Wenn der Cursor über einem Bezeichner ist, kann man sich mit `K`-Tastendruck Information über das bezeichnete Objekt anzeigen lassen. Mit `gd` kann man zur Definition, mit `gD` zur Deklaration und mit `gT` zur Typ-Definition springen. Durch `<Leertaste>+r` können Bezeichner auf intelligente Weise umbenannt werden. Dabei wird der Kontext berücksichtigt. So bleiben eventuelle Matches beispielsweise innerhalb eines Strings unangetastet.

### Intelephense-Lizenz
Nun muss leider gesagt werden, dass einige der genannten Features nur für zahlende Kunden zur Verfügung stehen. Intelephense verfolgt ein Freemium-Modell, bei dem einige LSP-Funktionen durch eine Lizenz freigeschaltet werden. Welche Funktionen davon betroffen sind, können der [offiziellen Webseite](https://intelephense.com/){: target="_blank"} entnommen werden. Dort kann eine lebenslang gültige Lizenz für 20€ erworben werden. Wenn man Intelephense viel nutzt, lohnt sich die Anschaffung wohl gerade für die Rename-Funktionalität.

Nun stellt sich natürlich die Frage, wie wir Neovim mitteilen, dass wir über die Lizenz verfügen.[^lizenz] Der erhaltene Lizenzschlüssel wird in einer Datei abgelegt. Im Rahmen von Lua wird der Schlüssel der Datei über eine Funktion entnommen:
```lua
...
```
Dieses Design hat natürlich den Vorteil, dass der Schlüssl nicht im Klartext in der Neovim-Konfiguration auftaucht. So gibt es keine Probleme, sie in einem öffentlichen Repository abzulegen.
...
Als nächstes können wir der `php.lua` Zeilen hinzufügen, durch die wir die geöffnete Datei per Tastenkombination auf Einhaltung der WordPress-Standards kontrollieren können.
```lua
vim.api.nvim_set_keymap('n', '<leader>lwp', [[:terminal ~/.config/composer/vendor/bin/phpcs --standard=WordPress %<CR><C-\><C-n>]], { noremap = true, silent = true })
```
Wenn wir die (selbst festgelegte) Leader-Taste und `lwp` (*lint WordPress*) drücken, öffnet sich ein neuer Buffer mit dem Ergebnis. Dieser Buffer kann mit `:bw` (*buffer wipe*) geschlossen werden. Dennoch ist `:b#` vorzuziehen, wodurch wir zum vorher geöffneten Buffer zurürckkehren.

`phpcbf` können wir mit Neovims hauseigenen Formatierungsfunktionalität verknüpfen.
```vim
set formatprg=phpcbf\ --standard=PSR2\ --ignore=*/vendor/*\ --ignore=*/tests/*

" Map <leader>cf to format the entire file
nnoremap <leader>cf :normal ggVGgq<CR>

" Map <leader>cs to format a selected block of code
vnoremap <leader>cs :normal gq<CR>
```
Durch die erste Zeile wird festgelegt: "The name of an external program that will be used to format the lines selected with the `gq` operator." So können wir *einzelne Zeilen* formatieren lassen. Mit den beiden anderen Zeilen erhalten wir weitere Keybindings, um die gesamte Datei zu formatiren oder die wir uns vielleicht einfach besser merken können.
...
Bezüglich WordPress ist die Situation vielleicht wenig überraschend. Die Schnittmenge zwischen Neovim-Enthusiasten und WordPress-Entwicklern und -Anwendern dürfte überschaubar sein. Die eine Seite repräsentiert einen DIY-Spirit, die andere Seite erwartet vorgefertigte Lösungen für eine möglichst schnelle und reibungslose Implementierung.
...
[^in-place]: Anders als bei ähnlichen Anwendungen wird die korrigierte Fassung nicht nur auf der Standardausgabe ausgegeben; die Zieldatei selbst wird angepast. Für eine bloße Ausgabe kann ein sogenannter Dry-Run ausgeführt werden: `phpcbf --dry-run path/to/your/directory`.
