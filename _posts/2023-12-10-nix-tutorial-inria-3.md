...
Im dritten Beitrag, [Packaging Your First Experiment](https://nix-tutorial.gitlabpages.inria.fr/nix-tutorial/first-experiment.html){:target="_blank"}, geht es um einen wissenschaftlichen Verwendungszweck von Nix. Wir lernen wie Experimente so Paketen zusammengefasst werden, dass sie perfekt wiederholt werden können. Die Einzelheiten dieses Vorgehens sind auch für angehende Nix-Enthusiasten von Interesse, die keine wissenschaftlichen Versuche durchzuführen beabsichtigen.
...
Zunächst sicherlich deshalb, weil es ein Grundprinzip der Programmierung verletzt: Don't repeat yourself. Dieses Prinzip wird der Aufhänger vom vierten Beitrag sein, ist aber hier gleichermaßen relevant. Die beiden Dateien enthalten eine Menge gleichen Code. Durch das Refactoring aus Beitrag 3 lässt sich einiger Boilerplate vermeiden.
...
Die Datei selbst, `default.nix`, spielt eine besondere Rolle im Nix-Ökosystem. In einer Note ganz am Ende vom zweiten Beitrag wird erklärt, dass `nix-shell` nach einer solchen Datei sucht, falls nicht explizit eine Datei angegeben wird (und falls nicht vorher eine `shell.nix` gefunden wird). Ähnlich verhalten sich andere Befehle und Funktionen, die wir im Folgenden kennenlernen.
...
Der Unterschied zum vorausgegangenen Ansatz besteht vor allem darin, dass die Datei eine Attributmenge repräsentiert und dass nun über Attribute auf die Derivation und die Shell-Umgebung zugegriffen werden kann. Da `chord` zu den `buildInputs` von `expEnv` gehört, muss es sich um ein *rekursives* Set handeln. Nur so können Attribute einer Attributmenge auf andere Attribute derselben Menge zugreifen.
...
```
- Skripte sind ausführbare Dateien. In der UNIX-Welt wird durch eine sogenannte Shebang-Zeichenfolge zu Beginn der Skriptdatei festgelegt, welchem Programm die Datei zur Ausführung übergeben wurde. In der obigen Datei, `runner.sh`, wurde Bash für die Interpretation verantwortlich (`#!/usr/bin/env bash`). Auch `nix-shell` kann als Interpreter dienen. Dabei kann sogar eine bestimmte Shell-Umgebung bestimmt werden, innerhalb von der das Skript ausgeführt werden soll.[^shebang] Hier das abgewandelte Skript (`runner_shebang.sh`):
```bash
